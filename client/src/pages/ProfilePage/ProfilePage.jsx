...
async function fetchJson(url, opts = {}) {
  ...
  if (!ct.includes("application/json")) throw new Error(`Non-JSON response (content-type: ${ct}). Preview: ${text.slice(0,300)}`);
...
function FriendCard({ f }) {
  const initials = (f?.name || f?.email || "U")
    ...
    .join("");
  return (
    <Link to={`/dashboard/profile?u=${encodeURIComponent(f.email)}`} className="block">
      <div className="bg-white/95 p-4 rounded-2xl shadow-lg transform transition hover:-translate-y-2 hover:shadow-2xl cursor-pointer">
        <div className="flex items-start gap-3">
          <div className="w-14 h-14 rounded-full grid place-items-center text-xl font-bold bg-gradient-to-tr from-indigo-100 to-pink-100 text-indigo-700 overflow-hidden">
            {f.image ? <img src={f.image} alt={f.name} className="w-full h-full object-cover" /> : initials}
          </div>
          <div className="flex-1">
            <div className="flex items-center justify-between">
              <div>
                <div className="font-semibold text-slate-800">{f.name || "Unnamed"}</div>
                <div className="text-xs text-gray-500">{f.native_language ? `${f.native_language} • ` : ""}{f.email}</div>
              </div>
              <div className="text-xs text-amber-500 font-semibold">{(f.badges?.length || 0) > 0 ? `${f.badges.length}★` : ""}</div>
            </div>
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!data?.success) throw new Error("Backend returned unexpected payload.");
        setUser(data.user);
      } catch (err) {
    ...
    (async () => {
      ...
      try {
        const results = await Promise.all(ids.map(async (id) => {
          try {
            const res = await fetch(`${BACKEND.replace(/\/$/, "")}/users/id/${encodeURIComponent(id)}`);
  ...
  const filteredFriends = useMemo(() => {
    ...
    return allFriends.filter(f => (f.name || f.email || "").toLowerCase().includes(q));
  ...
  if (loading) {
    return (
      <div className="min-h-screen grid place-items-center bg-gradient-to-br from-indigo-50 via-white to-pink-50 p-6">
        <div className="animate-pulse text-gray-600 font-medium">Loading profile…</div>
      </div>
    );
  }
  if (error) {
    return (
      <div className="min-h-screen grid place-items-center p-6 bg-gradient-to-br from-indigo-50 via-white to-pink-50">
        <div className="max-w-2xl w-full bg-white/90 p-6 rounded-lg shadow">
          ...
          <pre className="mt-2 text-xs text-gray-700 whitespace-pre-wrap">{error}</pre>
        </div>
      </div>
    );
  }
  ...
  const initials = (user.name || user.email || "U").split(" ").map(s => s[0]).slice(0,2).join("");
    ...
    <div className="min-h-screen pb-12 relative bg-gradient-to-br from-indigo-100 via-white to-pink-100">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <header className="flex flex-col md:flex-row items-start justify-between gap-6 mb-8">
          <div className="flex items-center gap-5">
            <div className="relative w-36 h-36 rounded-full p-1 bg-gradient-to-tr from-indigo-600 to-pink-500 shadow-2xl">
              ...
              <div className="absolute -bottom-2 -right-2 bg-white rounded-full px-2 py-1 text-xs font-semibold shadow-sm text-indigo-700">{user.role || "learner"}</div>
            </div>

            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold text-slate-800">{user.name || "Unnamed"}</h1>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col items-center">
              <div className="w-24 h-24 rounded-full grid place-items-center shadow-inner" style={{ background: `conic-gradient(#7C3AED ${progressPct * 3.6}deg, rgba(0,0,0,0.06) ${progressPct * 3.6}deg)` }}>
                <div className="w-14 h-14 rounded-full bg-white grid place-items-center text-sm font-semibold">{progressPct}%</div>
              </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="p-4 rounded-2xl bg-gradient-to-r from-indigo-500 to-pink-500 text-white shadow-lg transform hover:-translate-y-1 transition">
            <div className="flex items-center gap-3">
              <div className="bg-white/20 p-3 rounded-lg"><Clock /></div>
              <div>
                <div className="text-xs opacity-90">Sessions this week</div>
                <div className="text-lg font-semibold">{user.sessionsThisWeek ?? 0}</div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-400 to-cyan-400 text-white shadow-lg transform hover:-translate-y-1 transition">
            <div className="flex items-center gap-3">
              <div className="bg-white/20 p-3 rounded-lg"><User /></div>
              <div>
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-yellow-300 to-orange-400 text-white shadow-lg transform hover:-translate-y-1 transition">
            <div className="flex items-center gap-3">
              <div className="bg-white/20 p-3 rounded-lg"><Sparkles /></div>
              <div>
                <div className="text-xs opacity-90">Badges</div>
                <div className="text-lg font-semibold">{(user.badges?.length) || 0}</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm text-gray-500">{(user.friends?.length) || 0} friends</div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {friendsPreview.length ? friendsPreview.map(f => (
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="font-medium">{user.native_language || user.native || "Not set"}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Learning</div>
                <div className="font-medium">{Array.isArray(user.learning_language) ? (user.learning_language.join(", ") || "Not set") : (user.learning_language || "Not set")}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Proficiency</div>
                <div className="font-medium">{user.proficiency_level || "Not set"}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Interests</div>
                <div className="font-medium">{(user.interests && user.interests.length) ? user.interests.join(", ") : "Not set"}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Country</div>
                <div className="font-medium">{user.user_country || "Not set"}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">DOB</div>
                <div className="font-medium">{user.date_of_birth || "Not set"}</div>
              </div>
            </div>
          </div>
          ...
          <aside className="space-y-4">
            <div className="bg-white rounded-2xl p-4 shadow">
              <h4 className="text-sm font-semibold text-indigo-600 flex items-center gap-2">Badges <BadgeCheck className="text-amber-400" /></h4>
              <div className="mt-3 flex flex-wrap gap-2">
                {(user.badges && user.badges.length) ? user.badges.map((b,i) => (
                  <div key={i} className="px-3 py-1 rounded-full bg-gradient-to-r from-yellow-200 to-orange-300 text-xs font-semibold text-amber-800 shadow-sm">{b}</div>
                )) : <div className="text-sm text-gray-500">No badges yet</div>}
              </div>
            </div>

            <div className="bg-white rounded-2xl p-4 shadow">
              <h4 className="text-sm font-semibold text-indigo-600">Quick stats</h4>
              <div className="mt-3 text-sm text-gray-700 space-y-2">
                <div>Friends: <span className="font-medium">{(user.friends?.length) || 0}</span></div>
                <div>Followers: <span className="font-medium">{(user.followers?.length) || 0}</span></div>
                <div>Sessions this week: <span className="font-medium">{user.sessionsThisWeek ?? 0}</span></div>
              </div>
            </div>
          </aside>
        </section>
      ...
      {showFriendsModal && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-6">
          ...
          <div className="relative z-60 w-full max-w-4xl bg-white rounded-2xl shadow-2xl p-6 overflow-auto" style={{ maxHeight: "85vh" }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center bg-slate-100 rounded-full px-3 py-1">
                  ...
                  <input value={friendsQuery} onChange={(e) => setFriendsQuery(e.target.value)} placeholder="Search friends" className="bg-transparent outline-none text-sm" />
                </div>
                <button onClick={() => setShowFriendsModal(false)} className="p-2 rounded-full hover:bg-slate-100"><X /></button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {loadingFriends ? (
                ...
                filteredFriends.map(f => <FriendCard key={f._id || f.email} f={f} />)
              ) : (
                <div className="col-span-full text-center text-gray-500 p-8">No friends found.</div>
              )}
            </div>
          </div>
        </div>
