...
const SkeletonCard = () => (
  <div className="animate-pulse p-4 rounded-2xl bg-white shadow">
        ...
        const pairs = await Promise.all(
          accepted.map(async (s) => {
            try {
              ...
              const doc = json.feedback || json;
              const meResp =
                (doc.responses && (doc.responses[myEmail] || doc.responses?.[myEmail])) ||
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white shadow border w-full sm:w-auto justify-center"
          >
            <Clock size={16} /> Refresh
          </button>
          <button
            onClick={() => setTab("friends")}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-indigo-600 to-pink-500 text-white shadow w-full sm:w-auto justify-center"
            title="See friends"
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {loadingFriends ? (
                ...
                friends.map((f) => (
                  <div
                    key={f._id || f.email}
                    className="group relative p-4 rounded-2xl bg-white shadow hover:shadow-xl transform hover:-translate-y-1 transition"
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-14 h-14 rounded-full grid place-items-center text-lg font-bold bg-indigo-50 text-indigo-700">
                        {f.name
                          ? f.name
                              .split(" ")
                              .map((a) => a[0])
                    ...
                    <div className="mt-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2 items-center w-full sm:w-auto">
                        <button
                          ...
                          className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-gradient-to-r from-indigo-600 to-pink-500 text-white shadow w-full sm:w-auto justify-center"
                        >
                          <Send size={14} /> Request
                        </button>
                        <Link
                          to={`/dashboard/profile?user=${encodeURIComponent(f.email)}`}
                          className="text-xs text-gray-500 hover:underline px-3 py-2 rounded"
                        >
                          View
                        </Link>
                      </div>
            ...
            <div className="grid gap-4">
              {loadingSessions ? (
                ...
                incoming.map((s) => (
                  <div
                    key={s._id}
                    className="p-4 rounded-2xl bg-white shadow flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3"
                  >
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                      <button
                        ...
                        className="px-3 py-2 rounded bg-green-600 text-white inline-flex items-center gap-2 w-full sm:w-auto justify-center"
                      ...
                      <button
                        ...
                        className="px-3 py-2 rounded border inline-flex items-center gap-2 text-sm w-full sm:w-auto justify-center"
                    ...
                    >
                      <div className="w-full sm:flex-1">
                        ...
                        <div className="text-xs text-gray-500 mt-1">
                          Scheduled:{" "}
                          {s.scheduledAt ? new Date(s.scheduledAt).toLocaleString() : "No scheduled time"}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center w-full sm:w-auto">
                        <Link
                          ...
                          className="px-3 py-2 rounded bg-indigo-600 text-white inline-flex items-center gap-2 w-full sm:w-auto justify-center"
                        >
                          <Clock size={14} /> Start
                        </Link>
                        ...
                        {meSubmitted ? (
                          <button
                            className="px-3 py-2 rounded bg-gray-200 text-gray-700 inline-flex items-center gap-2 w-full sm:w-auto justify-center"
                          ...
                          <Link
                            ...
                            className="px-3 py-2 rounded bg-gradient-to-r from-indigo-600 to-pink-500 text-white inline-flex items-center gap-2 w-full sm:w-auto justify-center"
            ...
            <div className="grid gap-3">
              {loadingSessions ? (
                ...
                history.map((s) => (
                  <div
                    key={s._id}
                    className="p-3 rounded-2xl bg-white shadow flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3"
                  >
                    <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full grid place-items-center bg-indigo-50 text-indigo-700 font-bold">
            ...
            <div className="mt-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
              ...
              <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <button type="button" onClick={closeModal} className="px-4 py-2 rounded border w-full sm:w-auto">Cancel</button>
                <button type="submit" disabled={submitting} className="inline-flex items-center gap-2 px-4 py-2 rounded bg-gradient-to-r from-indigo-600 to-pink-500 text-white shadow w-full sm:w-auto justify-center">
