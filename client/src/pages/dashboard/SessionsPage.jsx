...
export default function SessionsPage() {
  ...
  const [message, setMessage] = useState("I'd like to practice speaking — are you available?");
  const [submitting, setSubmitting] = useState(false);

  const myEmail = authUser?.email || mongoUser?.email;

  // ---------- Friend loading ----------
  // We assume mongoUser.friends is an array of user _id strings
  useEffect(() => {
    let mounted = true;
    (async () => {
      ...
      try {
        ...
        // fetch each friend by id in parallel (calls /users/id/:id)
        const results = await Promise.all(
          friendIds.map(async (id) => {
            try {
              const res = await fetch(`${BACKEND}/users/id/${encodeURIComponent(id)}`);
              const json = await res.json();
              return json?.user ?? json; // be tolerant of shapes
            } catch (err) {
  ...
  const submitRequest = async (e) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json?.success) {
        // success - close modal and reload sessions
        closeModal();
        await loadSessions();
  ...
  const history = sessions.filter((s) => ["completed", "rejected", "cancelled"].includes(s.status) || (s.status === "accepted" && s.endTime));

  // ---------- optional: socket.io realtime (uncomment & configure) ----------
  // useEffect(() => {
  //   const socket = io(BACKEND, { query: { uid: authUser?.uid }});
  //   socket.on("newSession", () => loadSessions());
    ...
    <div className="p-6">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Book 1:1 speaking sessions with friends. Aim for 10+ minutes to count toward progress.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <button
            onClick={() => { loadSessions(); refreshMongoUser && refreshMongoUser(); }}
      ...
      <div className="mb-6 bg-white/60 rounded-2xl p-3 flex flex-wrap gap-2 items-center shadow-sm">
        {[
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg transition ${tab === t.key ? "bg-gradient-to-r from-indigo-600 to-pink-500 text-white shadow-lg" : "text-gray-700 hover:bg-gray-50"}`}
          >
            {t.label}
          </button>
        ))}
      ...
      <div>
          ...
          <section>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {loadingFriends ? (
                ...
                friends.map((f) => (
                  <div key={f._id || f.email} className="group relative p-4 rounded-2xl bg-gradient-to-br from-white to-white/90 shadow hover:shadow-xl transform hover:-translate-y-1 transition">
                    <div className="flex items-start gap-3">
                      <div className="w-14 h-14 rounded-full grid place-items-center text-lg font-bold bg-gradient-to-br from-indigo-100 to-pink-100 text-indigo-700">
                        {f.name ? f.name.split(" ").map(a => a[0]).slice(0,2).join("") : (f.email||"U")[0].toUpperCase()}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            <div className="font-semibold text-slate-800">{f.name || f.email}</div>
                            <div className="text-xs text-gray-500">{f.native_language ? `${f.native_language} • ` : ""}{f.email}</div>
                          </div>
                          <div className="text-xs text-gray-400">{f.status || "Offline"}</div>
                        </div>
                        <p className="mt-3 text-sm text-gray-600 line-clamp-3">{f.bio || "No bio yet."}</p>
                      </div>
                    </div>

                    <div className="mt-4 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 text-xs">Native: {f.native_language || "—"}</span>
                        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-indigo-50 text-indigo-700 text-xs">Learning: {(f.learning_language && f.learning_language[0]) || "—"}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <a href={`/profile/${encodeURIComponent(f.email)}`} className="text-xs text-gray-500 hover:underline">View</a>
                      </div>
                    </div>
                  </div>
                ))
              ) : (
                <div className="col-span-full p-6 text-center text-gray-500 bg-white rounded-2xl shadow">No friends yet — follow users to make friends.</div>
              )}
            </div>
          </section>
        )}
        ...
        {tab === "incoming" && (
          <section>
            <div className="grid gap-4">
              {loadingSessions ? <div>Loading...</div> : incoming.length ? incoming.map((s) => (
                <div key={s._id} className="p-4 rounded-2xl bg-gradient-to-r from-pink-50 to-white shadow flex justify-between items-center">
                  <div>
                    <div className="font-semibold">{s.fromName || s.fromEmail}</div>
                    <div className="text-sm text-gray-600">{s.message}</div>
                    <div className="text-xs text-gray-400 mt-2">Requested: {new Date(s.createdAt).toLocaleString()}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => acceptSession(s._id)} className="px-3 py-1 rounded bg-green-600 text-white inline-flex items-center gap-2"><Check size={14}/> Accept</button>
                    <button onClick={() => rejectSession(s._id)} className="px-3 py-1 rounded border inline-flex items-center gap-2 text-sm"><X size={14}/> Reject</button>
                  </div>
                </div>
        ...
        {tab === "outgoing" && (
          <section>
            <div className="grid gap-4">
              {loadingSessions ? <div>Loading...</div> : outgoing.length ? outgoing.map(s => (
                <div key={s._id} className="p-4 rounded-2xl bg-white shadow flex justify-between items-center">
                  <div>
                    <div className="font-semibold">To {s.toName || s.toEmail}</div>
                    <div className="text-sm text-gray-600">{s.message}</div>
                    <div className="text-xs text-gray-400 mt-2">Requested: {new Date(s.createdAt).toLocaleString()}</div>
                  </div>
                  <div><span className="px-2 py-1 rounded bg-yellow-100 text-yellow-800">Pending</span></div>
                </div>
        ...
        {tab === "upcoming" && (
          <section>
            <div className="grid gap-4">
              {loadingSessions ? <div>Loading...</div> : accepted.length ? accepted.map(s => (
                <div key={s._id} className="p-4 rounded-2xl bg-gradient-to-r from-emerald-50 to-white shadow flex justify-between items-center">
                  <div>
                    <div className="font-semibold">{s.fromEmail === myEmail ? `With ${s.toName || s.toEmail}` : `With ${s.fromName || s.fromEmail}`}</div>
                    <div className="text-xs text-gray-500">Scheduled: {s.scheduledAt ? new Date(s.scheduledAt).toLocaleString() : "No scheduled time"}</div>
                  </div>
                  <div className="flex gap-2">
                    <a href={`/meet?session=${s._id}`} className="px-3 py-1 rounded bg-indigo-600 text-white inline-flex items-center gap-2"><Clock size={14}/> Start</a>
                  </div>
                </div>
        ...
        {tab === "history" && (
          <section>
            <div className="grid gap-3">
              {loadingSessions ? <div>Loading...</div> : history.length ? history.map(s => (
                <div key={s._id} className="p-3 rounded-2xl bg-white shadow flex justify-between items-center">
                  <div>
                    <div className="font-medium">{s.fromEmail === myEmail ? `With ${s.toName || s.toEmail}` : `With ${s.fromName || s.fromEmail}`}</div>
                    <div className="text-xs text-gray-500">{s.status} • {s.endTime ? `Ended ${new Date(s.endTime).toLocaleString()}` : `Requested ${new Date(s.createdAt).toLocaleString()}`}</div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
          ...
          <form onSubmit={submitRequest} className="relative z-60 max-w-2xl w-full bg-white rounded-2xl p-6 shadow-xl">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={closeModal} className="text-gray-500 hover:text-gray-700"><X/></button>
            </div>
